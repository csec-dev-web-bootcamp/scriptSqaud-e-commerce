    ...
    <div>
      ...
      <ul className=" grid grid-cols-4  gap-3 ">
        {productData.map((product) => (
          <li
            key={product.id}
            className="  flex flex-col m-2 justify-center  border-slate-200 rounded-md p-2"
          >
            {/* Import and use the image for each product */}
            <Link href={`/detail/${product.id}`}>
              <div class="group my-10 flex w-full max-w-xs flex-col overflow-hidden rounded-lg  border-gray-100 bg-white shadow-md">
                <a
